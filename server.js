...
return { …base, Authorization: key }; // raw
}

async function bonzoFetch(path, opts = {}) {
const url = `${bonzoBase()}${path}`;
const headers = { …bonzoHeaders(), …(opts.headers || {}) };

// *** DIAGNOSTIC: log every Bonzo request so we can see what’s being sent ***
console.log(”[bonzoFetch] →”, opts.method || “GET”, url);
console.log(”[bonzoFetch] headers:”, JSON.stringify({
…headers,
Authorization: headers.Authorization
? headers.Authorization.slice(0, 30) + “…”  // truncate key in logs for safety
: undefined,
}));
...
// *** DIAGNOSTIC: log every Bonzo response ***
...
// —————— DEBUG ENDPOINT ——————
// Hit GET /debug-bonzo?id=98725114 in your browser to test auth live
// Remove this endpoint once auth is confirmed working.
...
console.log(”[debug-bonzo] mode:”, mode, “| base:”, base, “| keyPreview:”, keyPreview);
...
status: out.status,
ok: out.ok,
// Show first 1000 chars of body so we can see the actual error message
