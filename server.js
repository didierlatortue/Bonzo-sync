...
if (!out.ok) continue;
const data = out.json;
const list = data && (data.prospects || data.data || data.results || (Array.isArray(data) ? data : null)) || [];
...
return (out.json && out.json.results || []).map(function(x) { return x.person; }).filter(Boolean);
}
async function getPerson(resourceName, accessToken) {
const rn = ensurePeopleResourceName(resourceName);
const url = “https://people.googleapis.com/v1/” + rn +
...
const updateUrl = “https://people.googleapis.com/v1/” + rn + “:updateContact” +
...
const hdrs = Object.assign({ Authorization: “Bearer “ + accessToken, “Content-Type”: “application/json” },
etag ? { “If-Match”: etag } : {});
const r = await fetch(updateUrl, { method: “PATCH”, headers: hdrs, body: JSON.stringify(Object.assign({}, body, { etag })) });
const out = await readJsonOrText(r);
if (out.ok) { console.log(“Updated Google contact:”, out.json.resourceName); return out.json; }
...
return s.includes(“undeliverable”) || s.includes(“delivery has failed”) ||
s.includes(“delivery failed”) || s.includes(“returned mail”) ||
s.includes(“failure notice”) || s.includes(“mail delivery subsystem”) ||
s.includes(“delivery status notification”) || s.includes(“could not be delivered”) ||
s.includes(“message blocked”) || s.includes(“bounce”);
}
...
const data = await graphGet(”/users/” + encodeURIComponent(mailbox) + “/mailFolders/Inbox/messages?$top=5&$select=id,subject,receivedDateTime,from,bodyPreview”, token);
const messages = (data && data.value || []).map(function(m) {
return { id: m.id, subject: m.subject, receivedDateTime: m.receivedDateTime,
...
const token = await getMsGraphToken();
const data = await graphGet(”/users/” + encodeURIComponent(mailbox) + “/mailFolders/Inbox/messages?$top=” + top + “&$select=id,subject,receivedDateTime,from,bodyPreview”, token);
...
if (!email) { bounces.push({ id: m.id, subject: m.subject, extractedEmail: “”, action: “no_email_found” }); continue; }
const prospects = await bonzoFindProspectsByEmail(email);
if (!prospects.length) { bounces.push({ id: m.id, subject: m.subject, extractedEmail: email, action: “no_bonzo_match_found” }); continue; }
let cleanedCount = 0;
...
bounces.push({ id: m.id, subject: m.subject, extractedEmail: email, action: “clean_attempted”, bonzoMatches: prospects.length, cleanedCount });
}
